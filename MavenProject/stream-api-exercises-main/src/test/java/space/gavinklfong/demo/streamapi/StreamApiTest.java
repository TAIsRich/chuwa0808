...
public class StreamApiTest {
	...
	public void exercise1() {
		...
		List<Product> allItems = productRepo.findAll();
		log.info(String.format("all items %d", allItems.size()));
		allItems.stream().forEach(System.out::println);

		List<Product> result = allItems
			.stream()
			.filter(p -> p.getCategory().equalsIgnoreCase("Books"))
			.filter(p -> p.getPrice() > 100)
			.collect(Collectors.toList());
		log.info(String.format("result items %d", result.size()));
		...
		List<Order> result = orderRepo.findAll()
				...
				.filter(
					order -> order.getProducts().stream().anyMatch(
						product -> product.getCategory().equals("Baby")
					)
				)
				.collect(Collectors.toList());
