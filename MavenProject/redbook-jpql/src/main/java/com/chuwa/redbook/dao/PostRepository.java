...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    Post getPostByIDOrTitleWithJPQLNamedParameters(
            @Param("key") long id,
    ...
    @Query(value = "select * from posts p where p.id = :key or p.titile = :title", nativeQuery = true)
    Post getPostByIDOrTitleWithSQLNamedParameters(
            @Param("key") long id,
            @Param("title") String title);
}
