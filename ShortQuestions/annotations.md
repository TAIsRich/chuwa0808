...
21. ManyToMany
	...
	3. example: 
		1. @ManyToMany
    		2. Set<Student> likes;
22. @ResponseStatus
	1. usage: on exception class set http status code
	2. def: set a response status to a http status code
	3. example: 
...
26. @NamedQueries
	...
	3. example:
		...
		9. `})`
@Bean
	1. usage: use for config class defined a bean 
	2. def: defined a bean similar to @Component
	3. example: 
		1. `@Configuration`
...
@Component
	1. usage: on class that you want to spring to detect your bean
	2. def: Spring to automatically detect our custom beans
	3. example: 
		1. `@Component`
		2. `public class HibernateChuwa implements JpaChuwa {`
		3.    `@Override`
		4.    `public void printMessage() {`
		5.        `System.out.println("Message from " + getClass().getName());`
  		6. `}}`
@Controller
	1. usage: in controller class set it as controller
	2. def: RestController = @controller + @ResponseBody
	3. example:  
		1. `@Controller`
		2. `@ResponseBody`
		3. `@RequestMapping("/api/tree")`
		4. `public class TreeController {...} `
@ExceptionHandler
...
@RequestBody
	...
	3. example: 
		1. `@PostMapping()`
    		2. `public ResponseEntity<PostDto> createPost(@Valid @RequestBody PostDto postDto) {...}`
@Configuration
	1. usage: on config class usually use with @bean
	2. def:  to declare a classfor bean definitions
	3. example
		1. `@Configuration`
		2. `public class CommonConfig {`
    		3. `@Bean`
...
@EnableAutoConfiguration
	1. usage: on application class SpringBootApplication = @Configuration+@EnableAutoConfiguration+@ComponentScan
	2. def:  auto-configuring beans present in the classpath
	3. example: 
		1. `@Configuration`
		2. `@EnableAutoConfiguration(exclude={JdbcTemplateAutoConfiguration.class})`
		3. `public class EmployeeApplication {`
		4.    `public static void main(String[] args) {`
		5.        `ApplicationContext context = SpringApplication.run(EmployeeApplication.class, args);`
		6. `}}`
@ComponentScan
	1. usage: on config class 
	2. def: use to locate where to find bean file
	3. example: 
		1. `@Configuration`
		2. `@ComponentScan(basePackages = {"com.chuwa.springbasic"})`
